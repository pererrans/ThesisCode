function [exp_price_path all_price_paths all_util_paths counter] = futureExpectedPrice( MinesOpened, sim_dr, currentFirm, currentMine, sim_D_fluct, sim_D_prob, sim_Demand, sim_DPERM_change, sim_dperm,...
    t, T, decisions_in_dt, numFirms, numIncMines, el, SupplyCurve, rich_a, D_0, TotalIncentiveCurve, TotalIncCurve_byFirm, numYearsNPV, sim_orderOfFirms)
...
[market_p, market_q, cap_util, rewards, faces, firms_q, diag] = findPrice_new(T, numFirms, t, MinesOpened, sim_dperm, sim_DPERM_change, el, sim_D_prob, sim_D_fluct, sim_Demand(t), D_0, SupplyCurve(:,:,t), rich_a, TotalIncentiveCurve);
...
while(and(and(t<T, diff>conv_criteria), counter(1)<3))
    ...
    for(period = (t+1):T)
        ...
        if(mod(period, decisions_in_dt) == 0)
            [market_p, market_q, cap_util, rewards, faces, firms_q, diag] = findPrice_new(T, numFirms, period, MinesOpened_sc2, sim_dperm_sc2, sim_DPERM_change, el, sim_D_prob, sim_D_fluct, sim_Demand(period), D_0, SupplyCurve(:,:,t), rich_a, TotalIncentiveCurve);
...
%     all_price_paths(3, :)
 ...
 %% %%
%scenario 2 - incentive mines with opex that is equal or less than the price in the
...
while(and(and(t<T, diff>conv_criteria),counter(2)<3))
    ...
    for(period = t+1:T)
        ...
        %store it
        if(mod(period, decisions_in_dt) == 0)
            [market_p, market_q, cap_util, rewards, faces, firms_q, diag] = findPrice_new(T, numFirms, period, MinesOpened_sc3, sim_dperm_sc3, sim_DPERM_change, el, sim_D_prob, sim_D_fluct, sim_Demand(period), D_0, SupplyCurve(:,:,t), rich_a, TotalIncentiveCurve);
