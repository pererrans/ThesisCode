function handles = barweb(barvalues, errors, errors_low, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend, error_sides, legend_type)
...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
	...
	for i = 1:numbars
		...
		x = mean(x([1 3],:));
		handles.errors(i) = errorbar(x, barvalues(:,i), errors(:,i), errors_low(:,i), 'k', 'linestyle', 'none', 'linewidth', 2);
